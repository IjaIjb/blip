...
const VerticalFocusSection = () => {
  ...
  const verticals = [
    ...
    },
    {
      ...
      content: "We continually invest in state-of-the-art infrastructure and business technology to enable clients to process transactions and information services digitally and more efficiently. Our online travel and financial platform and flexible B2B, B2G offer clients secure, controlled, real-time access to bitable financial solutions. We offer our clients an omni channel experience which gives them access to a range of bitable financial channels, travel tools and solutions."
    },
  ];
    ...
    <section className="py-16  max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 mb-12">
        {verticals.map((vertical) => {
          ...
          return (
            <div 
              key={vertical.id} 
              className={`flex flex-col items-center transition-opacity duration-300 ${isInactive ? 'opacity-50' : 'opacity-100'}`}
            >
              <h3 
                className={`text-[#000000] font-[500] md:text-[32px] text-[20px] text-center ${isInactive ? 'text-gray-500' : ''}`}
              >
                {vertical.title}
              </h3>
              <div className='w-[140px] h-[140px] flex justify-center'>
                <img 
                  src={vertical.icon} 
                  alt={vertical.title} 
                  className={`transition-filter duration-300 ${isInactive ? 'grayscale' : ''}`}
                />
              </div>
              <button 
      ...
      {!activeVertical && (
        <div className="max-w-[950px] text-[24px] font-[400] mx-auto text-[#000000]">
          <p className="mb-4">
            At BLIP LLC our thinking digs deeper and sees further. It's an approach we call Solutionist
            Thinking: A can-do mindset that is considered and innovative, curious and resilient, with a
            passion for possibilities.
          </p>
          <p>
            It constantly challenges the conventional and embraces the power
            of collective thinking to unlock unique opportunities for our clients. It's the magic that
